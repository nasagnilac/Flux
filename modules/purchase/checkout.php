...
if (count($_POST) && $params->get('process')) {
	...
	if ($res) {
		...
		elseif ($deduct != $server->cart->getTotal()) {
			...
			$server->cart->clear();
			$session->setMessageData('Items have been purchased.  You may redeem them from the Delivery Man NPC at Alberta, Prontera, Geffen or Payon.');
		}
	}
	else {
